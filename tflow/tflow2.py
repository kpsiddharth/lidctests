...
def resize_input_image(x):
    ...
    # train the model.
    x_image = tf.reshape(x, [-1, 51, 51, 1])
    return x_image


x = tf.placeholder(tf.float32, shape=[None, 2601])
...
W_conv5 = weight_variable([7, 7, 256, 512])
...
num_neurons = 1024
W_fc1 = weight_variable([17 * 17 * 512, num_neurons])
b_fc1 = bias_variable([num_neurons])

h_pool_flat = tf.reshape(h_pool5, [-1, 17 * 17 * 512])
