...
with tf.Session() as sess:
    ...
    for i in range(3000):
        ...
        summary, _ = sess.run([merged, train_step], feed_dict={x: batch[0], y_: batch[1], keep_prob_1: 0.5, keep_prob_2: 0.5})
        train_writer.add_summary(summary, i)

    print('test accuracy %g' % accuracy.eval(feed_dict={
        x: dataset.test_images, y_: dataset.test_labels, keep_prob_1: 1.0, keep_prob_2: 1.0}))
    
    predictions = sess.run(y_conv, feed_dict={
        x: dataset.test_images, keep_prob_1: 1.0, keep_prob_2: 1.0})
    # print (predictions)
